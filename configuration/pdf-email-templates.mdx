...
<Note>
  ...
  visible on the form. Premium status is determined automatically by your
  subscription and cannot be set manually.
</Note>
...
| Document Type | Select | Yes | `Sales Invoice` or `Payment Entry` |
...
<table>
    <tbody>
        <tr>
            ...
            <td>
                <div><strong>Sello del CFDI:</strong> {{ tfd.SelloCFD }}</div>
                <div><strong>Sello del SAT:</strong> {{ tfd.SelloSAT }}</div>
            </td>
        </tr>
    </tbody>
</table>
