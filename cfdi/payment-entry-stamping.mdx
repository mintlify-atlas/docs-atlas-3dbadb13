...
The `get_cfdi_voucher` method on the `PaymentEntry` override builds a `cfdi40.Comprobante` of type **Pago**. Only the full-settlement path is supported — partial payments are not.
