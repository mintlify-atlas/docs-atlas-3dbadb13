...
<Note>
  The transition from **Pending Cancellation** to **Cancelled** is handled by the `check_cancellation_status` scheduled job, which runs hourly and polls the web service for any approved cancellations.
</Note>
...
The client also calls `tisp_apps.api.v1.cfdi.subscription_details` to check available stamp credits, which is exposed in ERPNext via the **Available Credits** button in CFDI Stamping Settings.
...
- **Stamp credits** — each successful stamp consumes one credit from your package. Check your balance via the **Available Credits** button in CFDI Stamping Settings.
