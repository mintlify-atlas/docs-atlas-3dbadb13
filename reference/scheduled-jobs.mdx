...
<Steps>
  ...
  <Step title="Handle NOT_CANCELLABLE">
    If `status.is_cancellable == CancellableStatus.NOT_CANCELLABLE`, the document is no longer eligible for cancellation. The method sets `mx_is_cancellable = 0` and `mx_cfdi_status = "Valid"`, then saves the document. No cancellation takes place.
  </Step>
  <Step title="Handle CANCELLED">
    If `status.status == DocumentStatus.CANCELLED`, the SAT has confirmed the cancellation. The method sets `mx_cfdi_status = "Cancelled"` and calls the document's internal `_cancel()` method, which cancels the ERPNext document and all linked entries.
  </Step>
</Steps>
