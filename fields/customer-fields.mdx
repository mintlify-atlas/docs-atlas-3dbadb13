...
The `Customer` override calls `is_valid_rfc()` from `erpnext_mexico_compliance.controllers.validators` every time a Customer record is saved, provided the customer's primary address is in Mexico and a Tax ID has been entered.
